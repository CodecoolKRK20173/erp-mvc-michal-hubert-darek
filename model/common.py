...
def quicksrt(table_to_srt, ascending_order = True):
    ...
    >>> quicksrt([1, 3, 5, 2, 8, 10], ascending_order = True)
    [1, 2, 3, 5, 8, 10]
    >>> quicksrt([1, 3, 5, 2, 8, 10], ascending_order = False)
    [10, 8, 5, 3, 2, 1]
    >>> quicksrt(['c', 'd' , 'a', 'b'], ascending_order = True)
    ['a', 'b', 'c', 'd']
    >>> quicksrt(['c', 'd' , 'a', 'b'], ascending_order = False)
    ['d', 'c', 'b', 'a']
    '''
    if not table_to_srt: return []
    if len(table_to_srt) == 1: return table_to_srt

    pivot_element = table_to_srt.pop(0)
...
def find_indx(list_of_elements, name_of_element):
    '''
    Try to find first element equal name_of_element in list_of_elements

    >>> find_indx([1,2,3,5,7], 3)
    2
    >>> find_indx(["g","j","b","c","a"], "c")
    3
    >>> find_indx([1,"2","b",5,"b"], "b")
    2
    >>> find_indx([1,2,3,5,7], 10)
    >>> find_indx([], 10)
    '''
    indx = 0
